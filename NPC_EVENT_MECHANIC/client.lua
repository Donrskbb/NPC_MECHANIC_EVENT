...
RegisterCommand("repair", function(source, args)
	TriggerServerEvent('mechanic:checkmoney')
end, false)
...
function GoToTarget(x, y, z, vehicle, driver, vehhash, target)
    ...
    ShowAdvancedNotification(companyIcon, companyName, "The mechanic has been informed", "Help has been sent to your breakdown location!. ~y~" .. companyName)
    enroute = true
    while enroute do
        Citizen.Wait(500)
...
function RepairVehicle(target, vehicle, driver)
    ...
    Citizen.Wait(500)
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "Bob" , mechPedPick.lines[math.random(#mechPedPick.lines)])   ----- Hizmet adı -----
	...
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "Pjoter" , "~b~Een fijne dag gewenst!")
end
...
function GetTargetVehicle(player, dir)
    ...
    if DoesEntityExist(dmgVeh) then
        ...
        ShowNotification("U moet zich in de buurt van het voertuig bevinden om ~o~ pechhulp te vragen.")       --- Aracın yanında değilse giden mesaj ----
    end
end
...
AddEventHandler('pagarmecano:success', function (price)
	...
	ShowNotification("Het bedrag is overgemaakt~r~-$" .. price .. "~s~.De ANWB Monteur is onderweg jou kant op!")
end)

RegisterNetEvent('pagarmecano:notenoughmoney')
AddEventHandler('pagarmecano:notenoughmoney', function (moneyleft)
	ShowNotification("~r~Je bent te arm, je hebt minimaal ~g~$" .. moneyleft .. "~s~ ~r~nodig.")
end)
...
AddEventHandler('pagarmecano:free', function ()
	...
	ShowNotification("Je auto is gratis ~y~schoongemaakt~s~!")
end)
-------------------
